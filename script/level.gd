...
func _deferred_reload_level() -> void:
	...
	if main.level == 1:
		...
		box_level_9.position = Vector2(876, 168)
		...
		box_level_10_1.position = Vector2(225, 168)
		...
		box_level_10_2.position = Vector2(876, 168)
		...
		box_1.position = Vector2(250, 290)
		add_child(box_1)
		...
		var box_1 = box.instantiate()
		box_1.position = Vector2(876, 168)
		add_child(box_1)
		allBoxes.append(box_1)
		box_1.bullet_hit.connect(_on_box_bullet_hit)
...
func _on_bulletfinder_body_entered(body: Node2D) -> void:
	...
	if allBirds.size() == 0 and main.level < 19:
		shootMove = false
		$win.play()
		main.level += 1
		call_deferred("check_level_and_birds")
	
	elif allBirds.size() == 0 and main.level == 19:
		shootMove = false
		$win.play()
